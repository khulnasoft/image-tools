...
bpftool-image: .buildx_builder
	PUSH=$(PUSH) EXPORT=$(EXPORT) TEST=true scripts/build-image.sh khulnasoft-bpftool images/bpftool linux/amd64,linux/arm64 "$$(cat .buildx_builder)" $(REGISTRIES)

llvm-image: .buildx_builder
	PUSH=$(PUSH) EXPORT=$(EXPORT) TEST=true scripts/build-image.sh khulnasoft-llvm images/llvm linux/amd64,linux/arm64 "$$(cat .buildx_builder)" $(REGISTRIES)
...
checkpatch-image: .buildx_builder
	PUSH=$(PUSH) EXPORT=$(EXPORT) scripts/build-image.sh khulnasoft-checkpatch images/checkpatch linux/amd64,linux/arm64 "$$(cat .buildx_builder)" $(REGISTRIES)
